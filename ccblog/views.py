...
def user_bloglist(request):
    userid = request.session.get('userid', default=None)
    blogtypeid = 0
    if request.GET.get('blogtypeid') is not None:
...
def user_blog(request):
    ...
    if userid is not None:
        ...
        if request.method == 'POST':
            ...
            if blogTypeId == 'null':
                return JsonResponse({"code": -1, "msg": "please choose blog type!"}, safe=False)
            if title == '':
                return JsonResponse({"code": -1, "msg": "title can not be empty!"}, safe=False)
            if content == '':
                return JsonResponse({"code": -1, "msg": "content can not be empty!"}, safe=False)

            if blogid == "0":
                newblog = Blog(userId=userid, title=title, content=content, blogTypeId=blogTypeId)
                newblog.save()
                return JsonResponse({"code": 1, "msg": "save success!"}, safe=False)
            else:
